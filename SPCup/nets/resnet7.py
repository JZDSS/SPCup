...
def build_net(x, n, is_training, num_classes):
    ...
    with tf.variable_scope('pre'):
        pre = layers.conv2d(inputs=x, num_outputs=16,  kernel_size = [7, 7], scope='conv',
                            weights_initializer=tf.truncated_normal_initializer(
                            stddev=math.sqrt(2.0 / 49 / shape[3])),
    ...
    h = layers.conv2d(inputs=h, num_outputs=num_classes, kernel_size=[1, 1], scope='fc1', padding='VALID',
                  weights_initializer=tf.truncated_normal_initializer(
                      stddev=math.sqrt(2.0 / 64 / num_classes)),
                  weights_regularizer=layers.l2_regularizer(0.0001),
                  normalizer_fn=layers.batch_norm,
                  normalizer_params={'is_training': is_training})

    return tf.reshape(h, [-1, num_classes])
